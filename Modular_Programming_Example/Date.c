...
static status_t is_date_valid(day_t day, month_t month, year_t year)
{
    if(day < 1 || day > 31)
    {
        puts("This day is not valid, please choose another day and try againg...");

        exit(EXIT_FAILURE);
    }
    else if(month < 1 || month > 12)
    {
        puts("This month is not valid, please choose another month and try again...");
    ...
    if((month == 1 || month == 3 || month == 5 || month == 7 || month == 8 || month == 10 | month == 12) && day <= 31)
        ...
        return (TRUE);
    else if(((month == 2 && day == 29) && (((year % 400 == 0) && (year % 100 == 0)) || ((year % 4 == 0) && (year % 100 != 0)))))
        return (TRUE);
    else if(month == 2 && day < 29)
        return (TRUE);
    else
    {
        puts("This is an INVALID DATE please choose another date and try again...");
